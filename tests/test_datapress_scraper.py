...
class DataPressScraperStub(DataPressScraper):

    def get_data(self):
        dirname = os.path.dirname(os.path.abspath(__file__))
        fixture_path = os.path.abspath(os.path.join(dirname, 'fixtures/datapress.json'))
...
class DataPressScraperTests(unittest.TestCase):
    ...
    def test_scraper(self):
        disable_stdout()
        stub = DataPressScraperStub('foo.bar/', 'X01000001', 'baz', 'csv', [], 'utf-8')
        stub.scrape()
        enable_stdout()
    ...
    def test_extra_fields(self):
        disable_stdout()
        stub = DataPressScraperStub('foo.bar/', 'X01000001', 'baz', 'csv', ['description'], 'utf-8')
        stub.scrape()
        enable_stdout()
